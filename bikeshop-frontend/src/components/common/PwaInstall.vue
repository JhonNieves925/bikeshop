<template>
  <Transition name="slide-up">
    <div v-if="visible" class="fixed bottom-0 left-0 right-0 z-50 p-3 safe-bottom">
      <div class="max-w-lg mx-auto bg-[#141414] border border-[#2a2a2a] rounded-2xl p-4 shadow-2xl flex items-center gap-3">
        <img :src="iconSrc" alt="BikeShop" class="w-12 h-12 rounded-xl flex-shrink-0" />

        <div class="flex-1 min-w-0">
          <p class="text-white text-sm font-semibold leading-tight">Instala BikeShop</p>
          <p v-if="!isIos" class="text-[#a0a0a0] text-xs mt-0.5">Accede más rápido desde tu pantalla de inicio</p>
        ...
        <button v-if="!isIos" @click="install"
          class="flex-shrink-0 bg-[#E31837] hover:bg-[#c51530] text-white text-xs font-bold px-4 py-2 rounded-xl transition-colors">
          Instalar
        </button>
...
const STORAGE_KEY = 'bikeshop-tienda-pwa-dismissed'
...
onMounted(() => {
  ...
  if (isIosDevice()) {
    isIos.value = true
    if (!sessionStorage.getItem(STORAGE_KEY)) setTimeout(() => { visible.value = true }, 3000)
    return
  }
